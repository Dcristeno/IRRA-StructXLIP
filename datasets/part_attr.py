...
from misc.part_attr_utils import (
    build_part_attribute_captions,
    default_part_attr_lexicon_path,
    load_part_attr_lexicon,
)
from utils.comm import get_world_size

from .bases import ImageDataset, ImageTextMLMDataset, TextDataset, tokenize


PART_NAMES = ("upper", "lower", "shoes")


class ImageTextPartAttrDataset(ImageTextMLMDataset):
    def __init__(self, dataset, args, transform=None, text_length=77, truncate=True):
        ...
        lexicon_path = args.part_attr_lexicon_file or str(default_part_attr_lexicon_path())
        self.lexicon = load_part_attr_lexicon(lexicon_path)
    ...
    def __getitem__(self, index):
        ...
        part_captions, part_masks = build_part_attribute_captions(caption, self.args, self.lexicon)
        for part_name in PART_NAMES:
            ret[f"{part_name}_caption_ids"] = tokenize(
                part_captions[part_name],
                tokenizer=self.tokenizer,
                text_length=self.text_length,
                truncate=self.truncate,
            )
            ret[f"{part_name}_mask"] = int(part_masks[part_name])
        return ret


def build_dataloader_part_attr(args):
    ...
    if args.training:
        ...
        train_set = ImageTextPartAttrDataset(
            dataset.train,
            args=args,
